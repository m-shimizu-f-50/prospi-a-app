...
現行アプリ(Laravel + Next.js)を静的解析した結果、**選手・大会のCRUDや成績集計まわりに、コードとDBスキーマの不整合による実行時エラーや、未実装のまま放置された箇所が多数ある**ことが分かった。「既存機能をSpring Bootに移植する」というより、「壊れた仕様を読み解いて設計し直す」作業に近い。

致命的な問題(詳細は [9. 改善点](#9-改善点)):

1. **バリデーション・モデルの定義とDBの実カラムが食い違っている**
   大会の新規作成・更新、成績の一括更新が、DBに存在しない列を読み書きしようとしており、実行時エラーになる可能性が高い(→ [6. DB構成](#6-db構成))。
2. **ルーティングだけ存在してコントローラーの実体がない**
   選手ごとの大会成績CRUD(`PlayerStatsController`)が丸ごと未実装(→ [7. API一覧](#7-api一覧))。
3. **フロント側にも未定義参照がある**
   選手の更新・削除、大会作成の一部の画面でビルド/実行時エラーになりうる(→ [3. 機能一覧](#3-機能一覧))。

リプレイスにあたって特に仕様確認が必要な点(詳細は [10. リプレイス時の注意点](#10-リプレイス時の注意点)):

- 大会タイプの分類(`rank/tournament` なのか `rank_battle/cup/league` なのか、コード上でも二重定義)
- 成績集計(勝率・打率などをDBで持つかフロントで都度計算するか)
- 認証・権限方針(Sanctumは入っているが実質未使用)
...
8. [主要ライブラリ](#8-主要ライブラリ)
9. [改善点](#9-改善点)
10. [リプレイス時の注意点](#10-リプレイス時の注意点)
11. [未確認・不明事項一覧](#未確認不明事項一覧)
12. [用語メモ](#用語メモ)
...
### 致命的(リプレイスで必ず解消すべき)
1. **モデル/バリデーションとDBスキーマの不整合**(根拠: [6. DB構成](#6-db構成))
...
2. **未実装のコントローラーをルーティングが参照**(根拠: [7. API一覧](#7-api一覧))
   - `PlayerStatsController` がファイルとして存在せず、対応する5つのAPIは呼び出すとクラス未検出エラーになる。
3. **フロントの未定義参照・欠落エクスポート**(根拠: [3. 機能一覧](#3-機能一覧))
...
4. **重複するモデル定義**(根拠: [5. ディレクトリ構成](#5-ディレクトリ構成))
...
5. **大会タイプの二重定義・不整合**(根拠: [6. DB構成](#6-db構成))
...
### 設計・保守性上の課題
6. **画面コンポーネントの肥大化**(根拠: [4. 画面一覧](#4-画面一覧)): `rank/page.tsx`(1064行)、`tournaments/page.tsx`(890行)が単一ファイルにUI・状態管理・API呼び出し・計算ロジックを内包しており、責務分離ができていない(CLAUDE.mdの「コンポーネントは責務を分離」方針に反する)。
7. **選手のCRUDが不完全**(根拠: [3. 機能一覧](#3-機能一覧)): 詳細取得・更新・削除が未実装のまま一覧・作成のみ先行しており、画面側にも編集・削除UIが見当たらない。
8. **認証機構(Sanctum)が導入されているが実質未使用**(根拠: [6. DB構成](#6-db構成)): ログイン画面が見当たらず、`/api/user` 以外は認証なしでアクセス可能。管理アプリとしてのアクセス制御方針が未確定と考えられる。
9. **フロントのUI用型とAPIレスポンス型に乖離**(根拠: [8. 主要ライブラリ](#8-主要ライブラリ)): `Batter`/`Pitcher`(UI用)は `homerun`, `rbi`, `strikeouts`, `games` を持つが、`usePlayerData.ts` ではAPIにその項目がないため常に `0` で初期化しており、実質機能していない。
10. **命名の不統一**(根拠: [6. DB構成](#6-db構成)): `tournaments`テーブルの `win`/`lose` のようなアプリ全体集計用らしきカラムと、`player_stats`の個別成績が混在しており、集計ロジックの置き場所(DB/バックエンド/フロント)が一貫していない(例: 打率・勝率はフロント側で都度計算)。
...
本文中で説明なしに使っている用語の簡単な補足(現行はLaravel/PHP製だが、リプレイス先のSpring Bootでも同じ概念が出てくるものが多い)。

| 用語 | 意味 |
|------|------|
| マイグレーション | DBのテーブル定義(カラム追加・削除等)をコードで管理し、順番に適用していく仕組み。Spring Bootでは Flyway / Liquibase が同じ役割。 |
| モデル(Model) | 1つのDBテーブルに対応するクラス。Laravelの `Eloquent` ORMの機能で、`$fillable`(一括代入を許可するカラム)や `$casts`(型変換ルール)などをここに定義する。Spring Bootでは JPA の `Entity` が近い。 |
| バリデーション | リクエストで送られてきたデータが、必須項目を満たしているか・型が正しいか等をチェックする処理。 |
| FK(外部キー) | 別テーブルの行を指し示すカラム(例: `player_stats.player_id` は `players.id` を指す)。参照先の行が消えたときの挙動(cascade delete等)もここで決まる。 |
| REST / RESTful | URLとHTTPメソッド(GET/POST/PUT/DELETE)の組み合わせでリソースを操作する設計スタイル。`POST /players` は良い例、`/players/create` のように動詞をパスに含めるのは非推奨とされる。 |
| ルーティング(routes) | URLパスとそれを処理するコントローラーのメソッドを対応づける設定。`routes/api.php` に定義があっても、対応するコントローラーの実体(クラス)がなければ実行時エラーになる(→ 本文の `PlayerStatsController` の例)。 |
| Sanctum | Laravel標準のAPI認証(トークン発行・検証)の仕組み。導入されているだけで実際に使われているかは別問題(本文の通り、現行はほぼ未使用)。 |
